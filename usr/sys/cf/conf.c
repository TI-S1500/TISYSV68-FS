...
#ident "@(#)conf.c	1.130 (TI) 92/09/29 ";
...
/*	lotsfree = max( min( max( <your value>, MEMSIZ/16),MEMSIZ/4),32+35);    */
/*	desfree  = max( min( max( <your value>, lotsfree/4), lotsfree/2,16+35); */
/*	minfree  = max( min( max( <your value>, desfree/4), desfree/2),8+35);   */
/*
...
extern s5setattr(), s5notify(), s5fcntl(), s5ioctl(), s5readmap(), s5igen();
...
extern hsinit(), hsiput(), hsiupdat(), hsreadi(), hsallocmap();
extern hsitrunc(), hsstatf(), hsnamei(), hsmount(), hsumount(),*hsfreemap();
extern hsopeni(), hsclosei(), hsupdate(), hsstatfs(), hsaccess(), hsgetdents();
extern hssetattr(), hsnotify(), hsfcntl(), hsioctl(), hsreadmap(), hsigen();
extern seltimein(), unselect();

extern struct inode *s5getinode();
extern struct inode *nfs_iget();
extern struct inode *s5iread();
extern struct inode *nfsiread();
extern struct inode *hsiread();
...
extern senetopen(), senetclose();
extern idp_open(), idp_close();
extern ipm_open(), ipm_close(); /* fjs */

/* DDP */
extern ddp_open(), ddp_close(), ddpinit();
extern struct streamtab ddpinfo;

/* ELAP */
extern elap_open(), elap_close(), elapinit();
extern struct streamtab elapinfo;

/* DDP pushable modules */
extern struct streamtab aspinfo;
extern struct streamtab atpinfo;

/* IPX */
extern	ipxopen(), ipxclose(), ipxinit();
extern struct streamtab ipxinfo;

/* IPXE pushable module */
extern	ipxeopen(), ipxeclose();
extern struct streamtab ipxeinfo;

/* SPX */
extern	spxopen(), spxclose(), spxinit();
extern struct streamtab spxinfo;

/* NCP */
extern	ncpopen(), ncpclose(), ncpinit();
extern struct streamtab ncpinfo;

/* NVT */
extern	nvtopen(), nvtclose();
extern struct streamtab nvtinfo;

/* NBIO */
extern	nbioopen(), nbioclose();
extern struct streamtab nbioinfo;

/* NBIX */
extern	nbixopen(), nbixclose();
extern struct streamtab nbixinfo;

/* CNL */
extern cnlopen(), cnlclose(), cnlread(), cnlwrite(), cnlioctl();

extern  do_systimer();
extern  zip_sched_getnetinfo();
extern  send_phony_reply();
extern  zip_handle_getmyzone();
extern  IpxScheduleRip();
extern  NvtSendSap();
extern  NvtResend();
extern  NvtKeepAlive();
extern  SpxTTimeOut();
extern  SpxRTimeOut();
extern  SpxWatchEmu();
extern  aarp_sched_req();
extern  NbTOKeepAlive();
extern  NbTOAddName();
extern  NbTONQForDG();
extern  NbTONQForSS();
extern  NbTOStartSession();
extern  NbTOHangUp();
extern  NbTOOrdHangUp();
extern  NbTOSessSend();
extern  NbTOAdaptReq();
extern  aspinit();
extern  atpinit();
...
extern struct streamtab senetinfo;
extern struct streamtab idpinfo;
extern struct streamtab ipminfo;		 /* fjs */
...
extern struct streamtab osi3info, osi4tmrinfo, osi4coxinfo;
extern struct streamtab qllcinfo, osi4clxinfo, x213info;

extern ropen(),  rclose(),  rstrategy(),  rprint(), rswap();
extern npopen(), npclose(), npstrategy(), npprint(), npswap();
extern ntopen(), ntclose(), ntstrategy(), ntprint();
extern nmtopen(), nmtclose(), nmtstrategy(), nmtprint();
extern mscopen(), mscclose(), mscstrategy(), mscprint(), mscswap();
extern mqtopen(), mqtclose(), mqtstrategy(), mqtprint();
extern mtopen(), mtclose(), mtstrategy(), mtprint();
extern mvbopen(), mvbclose(), mvstrategy(), mvswap();
...
extern mirbopen(), mirbclose(), mirstrategy(), mirprint(), mirswap();
...
extern pswopen(),   pswclose(),   pswread(),   pswwrite(),   pswioctl();
...
extern mirropen(),mirrclose(),mirread(),mirwrite(),mirioctl();

extern ckinit(), ckopen(),ckclose(),ckread(),ckwrite(),ckioctl();
extern tc_tty_open(),tc_tty_close(),tc_tty_read(),tc_tty_write(),tc_tty_ioctl();

extern spopen(), spclose();
extern ptmopen(), ptmclose(), ptsopen(), ptsclose();
extern ldraw_timeout();

extern slsopen(), slsclose();

extern mscinfo(), npinfo(), np2info(), mvinfo(), mirinfo();
...
extern  lckdioctl();
extern  senet_tmout();
extern  idp_riptimout();
extern  idp_peptimout();

drivlck_t item_sema = { 0,1,0x7f,0};/* initially unlocked. Used for q_link */
csema_t str_sema = { 0,1,0x7f,0};/* initially unlocked. Used for streams.  */
csema_t lockd_sema = {0,1,0x7f,0};/* nfs lockd semaphore. Initially unlocked.*/
...
/*25*/  senet_tmout, &str_sema,
/*26*/  idp_riptimout, &str_sema,
/*27*/  idp_peptimout, &str_sema,
/*28*/  idp_riptimout, &str_sema,
/*29*/  idp_peptimout, &str_sema,
/*30*/  do_systimer, &str_sema,          /* /pnw/driver/ddp/ti1500/sysglue.c  */
/*31*/  zip_sched_getnetinfo, &str_sema, /* /pnw/driver/ddp/ti1500/zip.c */
/*32*/  send_phony_reply, &str_sema,     /* /pnw/driver/ddp/ti1500/zip.c */
/*33*/  zip_handle_getmyzone, &str_sema, /* /pnw/driver/ddp/ti1500/zip.c   */
/*34*/  IpxScheduleRip, &str_sema,       /* /pnw/driver/ipx/ti1500/ipx.c   */
/*35*/  NvtSendSap, &str_sema,           /* /pnw/driver/nvt/ti1500/nvt.c   */
/*36*/  NvtResend, &str_sema,            /* /pnw/driver/nvt/ti1500/nvt.c   */
/*37*/  NvtKeepAlive, &str_sema,         /* /pnw/driver/nvt/ti1500/nvt.c   */
/*38*/  SpxTTimeOut, &str_sema,          /* /pnw/driver/spx/ti1500/spx.c   */
/*39*/  SpxRTimeOut, &str_sema,          /* /pnw/driver/spx/ti1500/spx.c   */
/*41*/  SpxWatchEmu, &str_sema,          /* /pnw/driver/spx/ti1500/spx.c   */
/*42*/  aarp_sched_req, &str_sema,        /* /pnw/driver/elap/ti1500/aarp.c */
/*43*/  NbTOKeepAlive, &str_sema,      /* /pnw/driver/nbio/ti1500/nb_queue.c */
/*44*/  NbTOAddName, &str_sema,
/*45*/  NbTONQForDG, &str_sema,
/*46*/  NbTONQForSS, &str_sema,
/*47*/  NbTOStartSession, &str_sema,
/*48*/  NbTOHangUp, &str_sema,
/*49*/  NbTOOrdHangUp, &str_sema,
/*50*/  NbTOSessSend, &str_sema,
/*51*/  NbTOAdaptReq, &str_sema
...
/* LOGGING */
/*
 If a new loggable device is defined then it's device
 id should be added to the table below and NUMBER_LOG_TYPES
 should be increased in conf.h.
 */
int loggable_devices[] =
{
	NPID,
	MSCDID,
	MVID,
...
/* 0*/	ropen,    rclose,    rstrategy,    rprint,    NULLSEMA, 0, NOID,rswap,
/* 1*/	npopen,   npclose,   npstrategy,   npprint,   NULLSEMA, 1, NPID,npswap,
/* 2*/	ntopen,   ntclose,   ntstrategy,   ntprint,   NULLSEMA, 0, NTID,nodev,
/* 3*/	mscopen,  mscclose,  mscstrategy,  mscprint,  NULLSEMA, 1, MSCDID,mscswap,
/* 4*/	mqtopen,  mqtclose,  mqtstrategy,  mqtprint,  NULLSEMA, 0, MSCQID,nodev,
/* 5*/	mtopen,   mtclose,   mtstrategy,   mtprint,   NULLSEMA, 0, MSCMID,nodev,
/* 6*/	nmtopen,  nmtclose,  nmtstrategy,  nodev,     NULLSEMA, 0, NMID,nodev,
/* 7*/	mvbopen,  mvbclose,  mvstrategy,   nodev,     NULLSEMA, 1, MVID,mvswap,
/* 8*/	nulldev,  nulldev,   nodev,   	   nodev,     NULLSEMA, 0, NOID,nodev,
/* 9*/	rdopen,   rdclose,   rdstrategy,   rdprint,   NULLSEMA, 0, NOID,nodev,
/* 10*/	nfsdopen, nfsdclose, nfsdstrategy, nfsdprint, NULLSEMA, 0, NOID,nodev,
/* 11*/	lckdopen, lckdclose, nodev, nodev, NULLSEMA, 0, NOID,nodev,
/* 12*/	mirbopen, mirbclose, mirstrategy,  mirprint,  NULLSEMA, 1, MRID,mirswap,
};
...
/*12*/	dpopen,	dpclose,dpread,	dpwrite,dpioctl,0,NULLSEMA,0,NOID,0,
...
/*74*/ senetopen,senetclose,nodev,nodev,nodev,0,&str_sema,0,NOID,&senetinfo,
/*75*/ nulldev,	nulldev,nodev,	nodev,	nulldev,0,NULLSEMA,0,NOID,0,
/*76*/ mirropen,mirrclose,mirread,mirwrite,mirioctl,0,NULLSEMA,1,MRID,0,
/*77*/ spopen,spclose,nodev,nodev,nodev,0,&str_sema,0,NOID,&spinfo,
/*78*/ pswopen,pswclose,pswread,pswwrite,pswioctl,0,NULLSEMA,0,NOID,0,
/*79*/ ckopen,ckclose,ckread,ckwrite,ckioctl,0,NULLSEMA,0,LINK_MAJOR,0,
/*80*/ tc_tty_open,tc_tty_close,tc_tty_read,tc_tty_write,tc_tty_ioctl,0,NULLSEMA,0,TTY_MAJOR,0,
/*81*/ nulldev,nulldev,nodev,nodev,nulldev,0,&str_sema,0,NOID,&osi3info,
/*82*/ nulldev,nulldev,nodev,nodev,nulldev,0,&str_sema,0,NOID,&osi4tmrinfo,
/*83*/ nulldev,nulldev,nodev,nodev,nulldev,0,&str_sema,0,NOID,&osi4coxinfo,
/*84*/ nulldev,nulldev,nodev,nodev,nulldev,0,&str_sema,0,NOID,&osi4clxinfo,
/*85*/ ipm_open,ipm_close,nodev,nodev,nodev,0,&str_sema,0,NOID,&ipminfo,
/*86*/ idp_open,idp_close,nodev,nodev,nulldev,0,&str_sema,0,NOID,&idpinfo,
/*87*/ nulldev,smpclose,smpread,nulldev,nulldev,0,NULLSEMA,0,NOID,0,
/*88*/ ipxopen,ipxclose,nodev,nodev,nodev,0,&str_sema,0,NOID,&ipxinfo,
/*89*/ spxopen,spxclose,nodev,nodev,nodev,0,&str_sema,0,NOID,&spxinfo,
/*90*/ ncpopen,ncpclose,nodev,nodev,nodev,0,&str_sema,0,NOID,&ncpinfo,
/*91*/ nvtopen,nvtclose,nodev,nodev,nodev,0,&str_sema,0,NOID,&nvtinfo,
/*92*/ ddp_open,ddp_close,nodev,nodev,nodev,0,&str_sema,0,NOID,&ddpinfo,
/*93*/ elap_open,elap_close,nodev,nodev,nodev,0,&str_sema,0,NOID,&elapinfo,
/*94*/ nbioopen,nbioclose,nodev,nodev,nodev,0,&str_sema,0,NOID,&nbioinfo,
/*95*/ cnlopen,cnlclose,cnlread,cnlwrite,cnlioctl,0,NULLSEMA,0,NOID,0,
/*
clone devices:
	/dev/ipx
	/dev/spx
...
struct fstypsw fstypsw[] =
{
	...
	nodev, s5ioctl, s5igen, nodev, nodev, nodev, nodev },
	...
	{   /* this is the CD-ROM entry */
	hsinit, hsiput, hsiread, nodev, hsiupdat, hsreadi, nodev, 
	hsitrunc, hsstatf, hsnamei, hsmount, hsumount, nodevi, 
	hsopeni, hsclosei, hsupdate, hsstatfs, hsaccess, hsgetdents, 
	hsallocmap, hsfreemap, hsreadmap, hssetattr, hsnotify, hsfcntl, 
	nodev, hsioctl, hsigen, nodev, nodev, nodev, nodev }
};
...
int slot;
{
	...
	if( (tmp = get_ctrldndx(slot)) == -1 )
		return(-1); 		    /* Unknown board/slot */

	switch(devmap.ctrld[tmp].ctrltype) {
...
/*
 * Table below is partially initialized at compile time.  Fields with -1s
 * are determined later...during system boot.
*/
struct	cr_index_info cr_index_info_tab[] =
{
	NPID,   -1, -1, npinfo,
	MSCDID, -1, -1, mscinfo,
	MSCDID, -1, -1, np2info,
...
int	cr_index_cnt = sizeof(cr_index_info_tab)/sizeof(struct cr_index_info);

/* Using ID, bmaj, or cmaj, find an index into cr_index_info_tab that points to
** the function in the correct device driver that finds all the miscellaneous
** information needed by various places that need to be able to get from
** from bmajor to info routine or cmajor to bmajor, etc.
*/
get_cr_index( id, bmajor, cmajor )
int  id, bmajor, cmajor ;
{
	int	tmp;

	for( tmp = 0; tmp < cr_index_cnt; tmp++ );
	{
		if( id	    &&	(id     == cr_index_info_tab[tmp].id) )
			return(tmp);
		if( bmajor  &&	(bmajor == cr_index_info_tab[tmp].bmajor) )
			return(tmp);
		if( cmajor  &&	(cmajor == cr_index_info_tab[tmp].cmajor) )
			return(tmp);
	}
	return(-1);
}
