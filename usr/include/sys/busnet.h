...
#ifdef	SCCSID
static char busnet_h[] = "@(#)busnet.h	1.36 (TI) 92/04/14";
#endif	SCCSID
...
/* COMM errors use some bits in the errno for special purposes.
   These should be masked off before logemsg is called. The
   following macro checks for a COMM error and masks off the
   bits if it is.
*/
#define CHKCOMERR(errno)              ((errno & 0xF000000) == 0x6000000) ? \
				       (errno & 0xFF000FFF) : \
                                        errno
