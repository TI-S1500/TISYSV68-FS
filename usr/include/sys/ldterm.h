...
*/
#ifdef SCCSID
static char ldterm_h[] = " @(#)ldterm.h	1.5 91/07/29";
#endif
/* NOTE: struct also defined in space.h - change one, change'm all */
...
struct ldterm
{
	...
	unsigned char ld_col;		/* output column of tty */
	unsigned char ld_erase;		/* erase char */
	unsigned char ld_kill;		/* line kill char */
	unsigned char ld_intrc;		/* interrupt char */
	unsigned char ld_quitc;		/* quit char */
	unsigned char ld_startc;		/* start char */
	unsigned char ld_stopc;		/* stop char */
	unsigned char ld_eofc;		/* end-of-file char */
	unsigned char ld_eol;		/* input delim char (like NL) */
	unsigned char ld_eol2;		/* 2nd input delim char */
	unsigned char ld_swtchc;		/* switch char for shl */
	unsigned char canonb[CANBSIZ];   /* input buffer for line formation */
	unsigned char *op;		/* pointer for input buffer */
