...
#ifdef	SCCSID
static char conf_h[] = "@(#)conf.h	1.20 (TI) 92/03/03";
#endif	SCCSID
...
/* LOGGING */
/*
 If a new loggable device is defined then NUMBER_LOG_TYPES
 should be increased and the device id should be added to
 loggable_devices in conf.c.
 */
#define NUMBER_LOG_TYPES 4
extern int loggable_devices[];
#define CHAR_FLAG 0x100000   /* Char/Block flag for MVID and MRID */
/* LOGGING */
...
/* Declaration of array structure used by various places to get from a
 * cmajor number to a bmajor number or from a bmajor to an info routine, etc.
*/
struct cr_index_info {
	int	id;
	short	bmajor;
	short	cmajor;
	int	(*infofunc)();	/* driver entry point for multi-
...
extern struct cr_index_info cr_index_info_tab[];
#endif KERNEL
...
/*27*/	int		(*fs_igen)();		/* generation number for NFS */
/*28*/	int		(*fs_fill[4])();
};
#ifdef KERNEL
