...
#ifdef	SCCSID
static char user_h[] = "@(#)user.h	1.34 (TI) 90/01/05";
#endif	SCCSID
...
#ifdef NU
	struct	smap u_odmap;		/* special smap for clone data */
	struct	smap u_osmap;		/* ditto for ustack */
#endif
...
/*									*/
	...
	struct {			/* info about the executable file */
		...
		unsigned sh_tsize;	/* shared lib text size */
		unsigned sh_tstart;	/* byte offset of shared text */

		unsigned ux_tsize;	/* program text size */
		unsigned ux_tstart;	/* byte offset of program text */

		unsigned sh_dsize;	/* shared lib data size */
		unsigned sh_dstart;	/* byte offset of shared data */
		unsigned sh_bsize;	/* shared lib bss size */
		...
		struct inode *sh_ip;	/* inode pointer for shared lib */
	} u_exdata;
