...
#ifdef	SCCSID
static char boot_h[] = "@(#)boot.h	1.21 (TI) 92/09/25";
#endif	SCCSID
 ...
 struct boot 
{
	...
	int	maxsegs;		/* max segs per process (pmaplist)*/
	...
	int	sdmamax;		/* max size of any sdma structure */
	...
	int     log_enable;	/* Log Enable                           */
	int	ktext;		/* kernel test size in pages            */
	int	kdata;		/* kernel data and bss size in pages    */
	char	*nvram_addr[MAXCPU];  /* nvram physical address         */
};
